/* 
Christmas jumper code
Plays a series of Christmas tunes activated by a button while simultaneously flashing LEDs in time with the music.
Code adapted from code by Tom de Simone (https://github.com/tomds/Arduino-Tunes) which plays Christmas songs through the speaker
12th December 2014
...
// Generate a tone by passing a square wave of a certain period to the buzzer
...
/* Code for using a button as a start/stop toggle
...
/* Take a string representing a tune and parse it to play the notes through the buzzer.
Parameters:
...
*/
void parseTune(char notes[], int beatLength) {
  boolean play = true;
  
  // 1 iteration of this loop == 1 note played
  for (int i = 0; notes[i] != '.' && play; i++) { // stop iteration if '.' is the next char
    updateButtonState();
    if (buttonState == LOW) { // For every note, check to see if the button has been pressed to stop the tune
      play = false;
    } else {
      if (notes[i] == ',') { // ',' signifies a rest
        ...
        alternateLeds(); // alternate the LEDs every note to make them "dance"
        char note = notes[i];
        boolean sharp;
...
// Write your tunes in here using the grammar described at the top of this file.
void playTune (int tune) {
  if (tune == 0) { // Jingle Bells
    ...
    parseTune(notes, beatLength);
  } else if (tune == 1) { // The Holly and the Ivy
    char notes[] = "g4g2g2g4E4D4b6g2g2g2g4E4D8D2C2b2a2g4b2b2e2e2d4g2a2b2C2b4a4g8,8.";
    parseTune(notes, beatLength * 1.50);
    ...
    parseTune(notes, beatLength * 1.25);
...
void loop() {
  /* Start off playing the first tune.
  Each button press played after a tune has ended plays the next song in the sequence (circular).
  To stop a tune press the button when it is playing. */
